...
var DrawHelper = (function() {
	...
	_.prototype.initialiseHandlers = function() {
		...
		function callPrimitiveCallback(name, position) {
			...
			// for GroundPrimitives, we store our decorated object in the id member
			if(pickedObject && pickedObject.primitive && pickedObject.id && pickedObject.id[name]) {
				pickedObject.id[name](position);
			}

			// otherwise, we can access the decorated primitve directly
			else if(pickedObject && pickedObject.primitive && pickedObject.primitive[name]) {
		...
		handler.setInputAction(
			...
			function (movement) {
				...
				if(!pickedObject && mouseOutObject && mouseOutObject.id){
					mouseOutObject.id.mouseOut(movement.position);
				}

				// GroundPrimiive
				if(pickedObject && pickedObject.id && pickedObject.primitive){

					if(mouseOutObject && mouseOutObject.id && (!pickedObject || mouseOutObject.primitive != pickedObject.primitive)) {
						!(mouseOutObject.id.isDestroyed && mouseOutObject.id.isDestroyed()) && mouseOutObject.id.mouseOut(movement.endPosition);
						mouseOutObject = null;
					}

					if(pickedObject && pickedObject.primitive && pickedObject.id) {

						if(pickedObject.id.mouseOut) {
							mouseOutObject = pickedObject;
						}
						if(pickedObject.id.mouseMove) {
							pickedObject.id.mouseMove(movement.endPosition);
						}
					}
				}
				...
				// Primitive
					...
					// if mouseOutObject is from GP
					if(mouseOutObject && mouseOutObject.id){
						mouseOutObject = null;
					}

					if(mouseOutObject && (!pickedObject || mouseOutObject.primitive != pickedObject.primitive)) {
						!(mouseOutObject.primitive.isDestroyed && mouseOutObject.primitive.isDestroyed()) && mouseOutObject.primitive.mouseOut(movement.endPosition);
						mouseOutObject = null;
					}
					...
					if(pickedObject && pickedObject.primitive) {
						...
						if(pickedObject.primitive.mouseOut) {
							mouseOutObject = pickedObject;
						}

						if(pickedObject.primitive.mouseMove) {
							pickedObject.primitive.mouseMove(movement.endPosition);
						}
					}
				}
	...
	_.prototype.setListener = function(groundPrimitive, type, callback) {
		groundPrimitive[type] = callback;
	}
					...
					if(this._markers == null) {
						...
						this._globeClickhandler.setInputAction(
							function (movement) {
								...
								if(!(pickedObject && !pickedObject.id.isDestroyed() && pickedObject.primitive)) {
	...
	function getDisplayLatLngString(cartographic, precision) {
		return Cesium.Math.toDegrees(cartographic.longitude).toFixed(precision || 2) + ", " + Cesium.Math.toDegrees(cartographic.latitude).toFixed(precision || 2);
	}
	...
	function setListener(groundPrimitive, type, callback) {
		groundPrimitive[type] = callback;
	}
